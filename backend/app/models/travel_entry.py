...
class PackingEntry(db.Model):
    ...
    def to_dict(self):
        return {
            ...
            'unit_price': float(self.packing_type.unit_price) if self.packing_type and self.packing_type.unit_price else 0,
            'quantity': self.quantity or 0,
            'subtotal': float(self.packing_type.unit_price) * (self.quantity or 0) if self.packing_type and self.packing_type.unit_price else 0,
            'remark': self.remark,
...
class TravelPersonDays(db.Model):
    ...
    def to_dict(self):
        return {
            ...
            'unit_price': float(self.travel_category.day_rates[0].unit_price) if self.travel_category and self.travel_category.day_rates else 0,
    ...
    def _calc_subtotal(self):
        if not self.travel_category or not self.travel_category.day_rates:
            return 0
        rate = self.travel_category.day_rates[0]
        pd = float(self.person_days) if self.person_days else 0
        up = float(rate.unit_price) if rate.unit_price else 0
        return pd * up
...
class TravelPersonTrip(db.Model):
    ...
    def to_dict(self):
        return {
            ...
            'unit_price': 0,   # 由 TravelPersonTripFee 提供
            'visa_fee': 0,      # 由 TravelPersonTripFee 提供
            'subtotal': 0,      # 计算属性，由视图层或服务层计算
            'remark': self.remark,
            'created_at': self.created_at.isoformat() if self.created_at else None,
            'updated_at': self.updated_at.isoformat() if self.updated_at else None,
        }
