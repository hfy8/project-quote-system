...
def sync_departments(app):
    """同步部门数据"""
    with app.app_context():
        from api_app.app import db
        from api_app.app.models import Department
...
def sync_organizations(app):
    """同步组织数据"""
    with app.app_context():
        from api_app.app import db
        from api_app.app.models import Organization
...
def sync_positions(app):
    """同步职位数据"""
    with app.app_context():
        from api_app.app import db
        from api_app.app.models import Position
...
def sync_employees(app):
    """同步员工数据"""
    with app.app_context():
        from api_app.app import db
        from api_app.app.models import Employee, User
...
def sync_users_from_employees(app):
    """从员工数据同步用户到 users 表，并关联员工ID"""
    with app.app_context():
        from api_app.app import db
        from api_app.app.models import User, Employee
...
def sync_all(app):
    ...
    try:
        sync_organizations(app)
        sync_departments(app)
        sync_positions(app)
        sync_employees(app)
        sync_users_from_employees(app)
...
def init_scheduler(app):
    ...
    # 每天凌晨 3 点清理过期消息
    scheduler.add_job(
        func=lambda: cleanup_expired_messages(app),
...
def cleanup_expired_messages(app):
    ...
    with app.app_context():
        from api_app.app import db
        from api_app.app.models import Message
