...
# 中文字体路径（相对路径，字体跟随项目）
import os
# fonts 在 backend_fastapi/fonts/，export_service.py 在 api_app/app/services/
# 即 __file__ 向上 3 级：services → app → api_app → backend_fastapi
...
def generate_version_files(quotation_id, version_no, snapshot_data, data=None, lang='zh'):
    ...
    # __file__ = api_app/app/services/export_service.py → 4 次 dirname 升到 backend_fastapi
    base_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(__file__)))), 'static', 'versions', f'quotation_{quotation_id}')
...
def generate_version_pdfs(quotation_id, version_no):
    ...
    # __file__ = api_app/app/services/export_service.py → 4 次 dirname 升到 backend_fastapi
    base_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(__file__)))), 'static', 'versions')
    os.makedirs(base_dir, exist_ok=True)
    ...
    try:
        ...
        from api_app.app.services.export_service import _generate_version_word, calculate_version_totals
    ...
    try:
        from api_app.app import db as _db
