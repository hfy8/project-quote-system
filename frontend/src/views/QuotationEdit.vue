    ...
    <div class="edit-card">
      <el-tabs v-model="activeTab">
        ...
        <el-tab-pane v-if="isEdit" label="模块管理" name="modules">
          ...
          <el-table :data="modules" border style="width: 100%; margin-top: 16px;">
            ...
            <el-table-column label="参与人员" width="150">
              <template #default="{ row }">
                <span class="participant-count">{{ row.participants?.length || 0 }} 人</span>
                <el-button size="small" link type="primary" @click="manageParticipants(row)">管理</el-button>
              </template>
            </el-table-column>
          ...
          <!-- 参与人员管理弹窗 -->
          <el-dialog v-model="participantDialogVisible" title="管理参与人员" width="800px">
            <div class="participant-manager">
              <el-table :data="currentParticipants" border size="small" max-height="300">
                <el-table-column prop="user_name" label="姓名" />
                <el-table-column prop="username" label="用户名" />
                <el-table-column label="操作" width="100" align="center">
                  <template #default="{ row }">
                    <el-button size="small" type="danger" @click="removeParticipant(row.id)">删除</el-button>
                  </template>
                </el-table-column>
              </el-table>
            ...
            <template #footer>
              <el-button type="primary" @click="participantDialogVisible = false">完成</el-button>
            </template>
          </el-dialog>
...
function checkParticipantSelectable(row) {
  return !currentParticipants.value.some(p => p.user_id === row.id)
}
...
async function addParticipantsConfirm() {
  ...
  try {
    await api.post(`/modules/${currentModuleId.value}/participants`, {
      user_ids: selectedParticipantUsers.value.map(u => u.id)
    })
    ElMessage.success('添加成功')
    addParticipantDialogVisible.value = false
    // 重新加载当前模块的参与人员
    const data = await api.get(`/modules/${currentModuleId.value}/participants`)
    currentParticipants.value = data || []
    // 重新加载模块列表（更新参与人员数量）
    await loadModules()
  } catch (error) {
...
async function removeParticipant(participantId) {
  try {
    await api.delete(`/modules/${currentModuleId.value}/participants/${participantId}`)
    ElMessage.success('移除成功')
    currentParticipants.value = currentParticipants.value.filter(p => p.id !== participantId)
    // 重新加载模块列表（更新参与人员数量）
    await loadModules()
  } catch (error) {
